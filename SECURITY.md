...
| **A05 Injection** | The serial device path is validated (`/dev/cu.*`, no shell metacharacters) before it reaches the root `pppd` command, since it may come from a user-writable pin file; `jornada usb pin` and the app's USB pane apply the same rule when writing that file. Device-supplied file names are neutralized before use as local paths. The USB doctor runs only `/usr/sbin/ioreg` with fixed arguments and parses its plist output; nothing read from the registry is ever executed. |
